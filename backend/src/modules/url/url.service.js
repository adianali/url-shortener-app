...
async function listUrls(userId, { page = 1, limit = 20, filter } = {}) {
  ...
  const [urls, total] = await Promise.all([
    prisma.url.findMany({ where, orderBy: { createdAt: 'desc' }, skip, take: limit, select: URL_SELECT }),
    prisma.url.count({ where }),
  ]);

  return { urls, meta: { page, limit, total, totalPages: Math.ceil(total / limit) } };
}

async function getUrlById(id, userId) {
  const url = await prisma.url.findFirst({ where: { id, userId, ...ACTIVE_FILTER }, select: URL_SELECT });
  if (!url) {
...
async function updateUrl(id, userId, data, actorEmail) {
  const url = await prisma.url.findFirst({ where: { id, userId, ...ACTIVE_FILTER } });
  if (!url) {
    const err = new Error('URL not found');
    err.code = 'URL_NOT_FOUND';
    throw err;
  ...
  const updated = await prisma.url.update({ where: { id }, data: updateData, select: URL_SELECT });
...
async function getDashboard(userId) {
  const [totalUrls, totalClicks, recentUrls] = await Promise.all([
    ...
    prisma.click.count({ where: { url: { userId }, deletedAt: null } }),
    prisma.url.findMany({
...
module.exports = { createUrl, listUrls, getUrlById, updateUrl, deleteUrl, getDashboard, cacheUrl, getCachedUrl };
